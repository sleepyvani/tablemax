...
class MariaDBResultStream : public IResultStream {
    vector<ColumnInfo> cols_;
    vector<Row> rows_;
    QueryMeta meta_;
    size_t pos_ = 0;
public:
    MariaDBResultStream(vector<ColumnInfo> c, vector<Row> r, QueryMeta m)
...
class MariaDBPlugin : public IDbPlugin {
    MYSQL* conn_ = nullptr;
    string cs_, error_;
    ...
    ConnParams parse_url(const string& url) {
        ...
        if (at != string::npos) {
            ...
            s = s.substr(at + 1);
            auto col = auth.find(':');
            if (col != string::npos) { p.user = auth.substr(0, col); p.pass = auth.substr(col + 1); }
            else p.user = auth;
        }
        auto slash = s.find('/');
        if (slash != string::npos) { p.db = s.substr(slash + 1); s = s.substr(0, slash); }
        auto col = s.find(':');
        if (col != string::npos) { p.host = s.substr(0, col); p.port = atoi(s.substr(col + 1).c_str()); }
        else p.host = s;
...
public:
    ~MariaDBPlugin() override { disconnect(); }

    string name() const override { return "MariaDB"; }
    string version() const override { return "0.2.0"; }
    string db_type() const override { return "mariadb"; }
    ...
    void disconnect() override { if (conn_) { mysql_close(conn_); conn_ = nullptr; } }
    ...
    unique_ptr<IResultStream> execute(const string& query) override {
        ...
        auto t0 = chrono::steady_clock::now();
        if (mysql_query(conn_, query.c_str()) != 0) { error_ = mysql_error(conn_); return nullptr; }
        auto t1 = chrono::steady_clock::now();
        ...
        if (res) {
            ...
            for (int i = 0; i < ncols; ++i) {
                ColumnInfo ci; ci.name = fields[i].name; ci.type = "text";
        ...
        return make_unique<MariaDBResultStream>(move(cols), move(rows), meta);
    }
    ...
    vector<string> list_databases() override {
        ...
        if (res) { MYSQL_ROW row; while ((row = mysql_fetch_row(res))) if (row[0]) dbs.push_back(row[0]); mysql_free_result(res); }
        return dbs;
    }
    ...
    vector<string> list_tables(const string& database) override {
        ...
        if (res) { MYSQL_ROW row; while ((row = mysql_fetch_row(res))) if (row[0]) tables.push_back(row[0]); mysql_free_result(res); }
        return tables;
    }
    ...
    vector<ColumnInfo> get_table_schema(const string& table) override {
        ...
        if (mysql_query(conn_, ("DESCRIBE `" + table + "`").c_str()) == 0) {
            MYSQL_RES* res = mysql_store_result(conn_);
            if (res) {
                MYSQL_ROW row;
                while ((row = mysql_fetch_row(res))) {
                    ColumnInfo ci;
                    ci.name = row[0] ? row[0] : ""; ci.type = row[1] ? row[1] : "";
...
extern "C" {
    tablemax::IDbPlugin* create_plugin() { return new tablemax::MariaDBPlugin(); }
